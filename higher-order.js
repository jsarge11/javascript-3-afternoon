...
let evenNumbers  = mixedNumbers.filter((item) => item % 2 === 0)
...
//Code Here
let totalPopulation  = populations.reduce((prev, item) => prev + item)
...
let ordersTotal = orders.map((item) => item.tax * item.price + item.price)
...
let bobsTotal = purchases.reduce((acc, person) => {
  if (person.owner === 'Bob') {
    return acc + person.price;
  }
  else {
    return acc;
  }
}, 0) 
//let bobsTotal = purchases.filter((item)=>item.owner === "Bob").reduce((prev, curr) => (prev + curr.price),0);
